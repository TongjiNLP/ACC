...
from transformers import (
    BertTokenizer,RobertaTokenizer,BasicTokenizer,RobertaTokenizerFast
)
...
def get_squad_examples(
    ...
    for example_id,question,context in zip(examples["id"],examples["question"],examples["context"]):
        # 取出部分正确答案
        partial_answers=all_partial_answers[example_id]
        ...
        for i,ans in enumerate(partial_answers):
            # 处理问题
            new_question=f"Based on the prediction ` {ans} ` , "+question
...
def convert_examples_to_features(examples, tokenizer, max_seq_length,
                                 ...
                                 sequence_a_segment_id=0, sequence_b_segment_id=0,
    ...
    for (example_index, example) in enumerate(examples):
        ...
        #     print('Converting %s/%s pos %s neg %s', example_index, len(examples), cnt_pos, cnt_neg)
